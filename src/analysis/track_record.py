...
class TrackRecordStats:
    ...
    by_regime: list[CategoryWinRate] = field(default_factory=list)
    by_session: list[CategoryWinRate] = field(default_factory=list)
...
class TrackRecord:
    ...
    def compute(self, outcomes: list[TradeOutcome], min_trades: int = 5) -> TrackRecordStats:
        ...
        Args:
            ...
            min_trades: Minimum trades per category to include in filtered lists.
        ...
        # By sector
        by_sector_raw = self._group_by(outcomes, "sector")
        by_sector = [c for c in by_sector_raw if c.total >= min_trades]
        # By conviction
        by_conviction_raw = self._group_by(outcomes, "conviction")
        by_conviction = [c for c in by_conviction_raw if c.total >= min_trades]

        # By regime (only outcomes with non-None regime_at_entry)
        regime_outcomes = [o for o in outcomes if getattr(o, "regime_at_entry", None) is not None]
        by_regime_raw = self._group_by(regime_outcomes, "regime_at_entry") if regime_outcomes else []
        by_regime = [c for c in by_regime_raw if c.total >= min_trades]

        # By session (only outcomes with non-None session_at_entry)
        session_outcomes = [o for o in outcomes if getattr(o, "session_at_entry", None) is not None]
        by_session_raw = self._group_by(session_outcomes, "session_at_entry") if session_outcomes else []
        by_session = [c for c in by_session_raw if c.total >= min_trades]

        # Best/worst sector (from unfiltered data, always populated)
        best_sector = None
        worst_sector = None
        if by_sector_raw:
            sorted_sectors = sorted(by_sector_raw, key=lambda c: c.avg_pnl_pct, reverse=True)
    ...
    def format_for_prompt(stats: TrackRecordStats) -> str:
        ...
        if stats.by_conviction:
            ...
            for c in sorted(stats.by_conviction, key=lambda x: x.value):
                conv_parts.append(f"{c.value}: {c.win_rate_pct:.0f}%W ({c.avg_pnl_pct:+.1f}%, n={c.total})")
            lines.append(f"By conviction: {', '.join(conv_parts)}")

        # Regime breakdown
        if stats.by_regime:
            regime_parts = []
            for c in sorted(stats.by_regime, key=lambda x: x.value):
                regime_parts.append(f"{c.value}: {c.win_rate_pct:.0f}%W ({c.avg_pnl_pct:+.1f}%, n={c.total})")
            lines.append(f"By regime: {', '.join(regime_parts)}")

        # Session breakdown
        if stats.by_session:
            session_parts = []
            for c in sorted(stats.by_session, key=lambda x: x.value):
                session_parts.append(f"{c.value}: {c.win_rate_pct:.0f}%W ({c.avg_pnl_pct:+.1f}%, n={c.total})")
            lines.append(f"By session: {', '.join(session_parts)}")
