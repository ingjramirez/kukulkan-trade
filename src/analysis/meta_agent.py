...
class MetaAgentRunner:
    ...
    def _build_prompt(self) -> str:
        ...
        return (
            ...
            "- Trading agent prompts in src/agent/claude_invoker.py (session prompts, reasoning instructions)\n"
            "- Trading agent instructions in data/agent-workspace/CLAUDE.md\n"
            "- Context template in src/orchestrator.py (_write_context method — what the agent sees each session)\n"
            ...
            "- Any other source file you think could improve returns\n"
            "\n"
            "SECURITY — ABSOLUTE RULES (violation = instant rejection):\n"
            "- NEVER expose, log, print, or hardcode API keys, secrets, tokens, or credentials\n"
            "- NEVER modify .env, .env.example, or any file containing encryption keys\n"
            "- NEVER disable auth, JWT validation, or CORS protections\n"
            "- NEVER commit credentials or secrets to git\n"
            "- NEVER send secrets to external services, URLs, or logs\n"
            "Everything else is fair game. You can modify ANY source file to improve performance.\n"
            "\n"
            "RULES:\n"
