    ...
    async with db.session() as s:
        ...
        for i, d in enumerate([
            ...
            s.add(DailySnapshotRow(
                ...
                cash=30000.0, positions_value=3000 + (i * 100),
                daily_return_pct=0.3,
            ))
            s.add(DailySnapshotRow(
                portfolio="B", date=d,
                total_value=66000 - (i * 200),
                cash=55000.0, positions_value=11000 - (i * 200),
                daily_return_pct=-0.3,
            ))

        # Trades
        s.add(TradeRow(
            portfolio="A", ticker="XLK", side="BUY", shares=50,
            price=200.0, total=10000.0, reason="Momentum",
            executed_at=datetime(2026, 2, 3, 15, 0),
        ))
        s.add(TradeRow(
            portfolio="B", ticker="MSFT", side="BUY", shares=20,
            price=400.0, total=8000.0, reason="AI: tech thesis",
            executed_at=datetime(2026, 2, 4, 10, 0),
        ))
        s.add(TradeRow(
            portfolio="B", ticker="GLD", side="SELL", shares=30,
            price=180.0, total=5400.0, reason="AI exit: gold weakness",
...
async def test_report_handles_no_trades(db, mock_notifier):
    ...
    async with db.session() as s:
        for d in [date(2026, 2, 2), date(2026, 2, 6)]:
            s.add(DailySnapshotRow(
                portfolio="A", date=d,
                total_value=33000.0, cash=33000.0, positions_value=0.0,
            ))
        await s.commit()
